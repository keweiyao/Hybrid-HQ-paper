...
heavy quarks scatterings with medium partons through perturbative matrix-elements, while between scatterings heavy quarks are evolved with Langevin equations using an empirical diffusion constant.
...
The calibration involves valuating a multi-stage and multi-component model of the medium plus heavy flavor evolution for arbitrary values of parameters a high dimensional parameter space.
Its complexity and the dimensionality make it impossible to be tuned by hands.
...
We calibrate the model parameters in section \ref{section:calibration} and with high likelihood parameter sets novel observables are predicted in section \ref{section:prediction}. 
...
\begin{eqnarray}
f_{q,\bar{q}, g}(p) = \exp\left(-\frac{p \cdot u(x)}{T(x)}\right),
\end{eqnarray}
...
We always calculate $P$ in the center-of-mass frame of local fluid cell defined with temperature $T$ and velocity $u^\mu$.
...
The probability for heavy quark undergoes reaction of a certain channel inside the fluid cell per unit time or the so called scattering rate is,
\begin{eqnarray}\label{eq:rate}
    \frac{dP}{dt} &=& R(E_1, T, t) \nonumber \\
...
If a scattering process happens within $dt$ according to the probability $P=Rdt$, the details of the initial and final states can be obtained by sampling the differential scattering rate over the $(m+n)$-body phase space.
...
It has been shown that at leading order in the coupling constant, an energetic parton can scatter elastically with a medium parton (light quark and anti-quark or gluon) or emit collinear gluon triggered by multiple soft collisions which we call an inelastic process based on its particle number changing nature \cite{Arnold:2002zm}.
...
But the thermal HTL propagator involves a complicated self energy that depends on the medium reference frame, so it is hard for us to apply it in a cross-section based Monte-Carlo approach where it is easiest to calculate and sample the two- or three-body final states in the center of mass frame of the collision.
...
For now, we only consider the case of gluon emission with one collision with medium particle, the effective treatment of multiple collision will be discussed later.
...
Although the inelastic diagrams seem to be one order higher in coupling constant, because of the divergence of soft gluon emission that we screened by the asymptotic gluon thermal mass $m_D/\sqrt{2}$, it actually contributes at leading order as elastic processes \cite{Ghiglieri:2015ala}.
...
This typical time scale is called the gluon formation time $\tau_f$, during which the effects of multiple collisions add up coherently in a destructive manner to suppress the gluon emission spectrum \cite{Wang:1994fx, Baier:1996kr, Zakharov:1996fv}, known as the LPM effect .
...
A proper way out that worth study in future Monte Carlo implementation is to treat this long-lived heavy quark and primitive gluon system as a continuum specie of "particles" that can transport within its in-medium life time and scatter with medium partons \cite{ColemanSmith:2012vr}.
...
Additional, we consider the possibility of non-perturbative contribution to the diffusion component, which should be strong at low energy and low temperature and we arrive at the following simple ansatz,
\begin{eqnarray}
...
All the calculation in this section uses $\mu T = \pi T$.
...
\end{eqnarray}
which is the fraction deviation of the effective temperature from the temperature of the thermal bath.
...
Therefore for a thin medium, the phase space for gluon radiation is very restricted $k < LT^2$, and this is also the typical amount of energy lose per radiation.
Multiplied by an estimated number of collisions $N \propto LT$ the inelastic energy loss scales as $\Delta E \sim L^2T^3$.
On the other hand, for a thick medium, a heavy quark could have radiated multiple times $N \propto L/\tau_f$ and each time the radiation carries off a typical amount of energy. 
...
The initial spectra of the charm and bottom are parametrized as the one showed in [].
...
\item[2.] A collision-less Boltzmann equation (freestreaming) models the pre-equilibrium stage before the onset of hydrodynamics expansion \cite{Liu:2015nwa}.
...
The difficulties for applying this directly event-by-event heavy-ion collision simulation are that the model itself is computationally heavy and that $\mathcal{O}(10^4)$ minimum-biased events are needed to get statistical error of the simulation under control.
...
The solution is to use advanced sampling technique to sample $\mathcal{O}(100)$ design parameter sets in the parameter space and calculate the corresponding model outputs.
Then, a Gaussian process emulator fast interpolates the model outputs with estimated uncertainty at arbitrary point in the parameter space \cite{Rasmussen:2006gp}.
\begin{center}
\begin{table}[h]
...
So in the sense of calculating a leading order transport coefficient, we justifies ourselves to use Equation \ref{eq:qhat}, and this choice is also conceptually cleaner to compare with other pQCD based calculations.
