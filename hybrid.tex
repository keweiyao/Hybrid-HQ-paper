...
\caption{Calculation of heavy-flavor $R_{AA}$ with the high likelihood parameter set compared to ALICE D-meson Pb+Pb and p+Pb \cite{Abelev:2014hha,Abelev:2014hha} measurements. B-meson $R_{AA}$ are predicted.}\label{plots:pred:raa}
\end{figure*}
\begin{figure*}
...
\end{figure}
We first investigate the nuclear modification factor in a larger $p_T$ range with $|y|<0.5$.
In Figure \ref{plots:pred:raa} first three plots, B-meson $R_{AA}$ for 0-10\%, 30-50\%, and 60-80\% centrality is shown and compared to D-meson calculations and measurements for a reference.
The mass effect clearly separates $B$-meson from $D$-meson $R_{AA}$ in the intermediate $p_T$ range. 
The calculation for $p+Pb$ collisions is shown in the right most plot compared to ALICE measurement of $D$-meson $R_{AA}$ \cite{Abelev:2014hha}.
...
Next, we validate the calibrated model by comparing to CMS measured $D$-meson $v_3$ and make a prediction for B-meson $v_n$ in Figure \ref{plots:pred:vn}.
